...
```
运行MyApplication，从最终输出的结果，我们可以看到，相关代理和注解的一切信息，都可以在最终业务实现上获得
```xml
------------------------------------------------------------------------------------------
My Interceptor 1 :
   proxyClassName=org.springframework.aop.framework.ReflectiveMethodInvocation
   className=com.nepxion.matrix.proxy.example.simple.service.MyService1Impl
   classAnnotations=
      @org.springframework.stereotype.Service(value=myService1Impl)
   interfaceName=com.nepxion.matrix.proxy.example.simple.service.MyService1
   interfaceAnnotations=
      @com.nepxion.matrix.proxy.example.simple.aop.MyAnnotation1(description=MyAnnotation1, name=MyAnnotation1, label=MyAnnotation1)
   methodName=doA
   methodAnnotations=
   arguments=
      A
